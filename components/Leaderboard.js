...
export default function Leaderboard() {
  ...
  function PlayerRow({ player, i, prize }) {
    ...
    const isTop3 = i < 3
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr auto', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}>{name}</span>
        </div>
  ...
  return (
    <div style={{ maxWidth: '780px', margin: '32px auto 0', padding: '0 4px' }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: 700, marginBottom: '8px' }}>This Week's Prize Pool</div>
          <div style={{ fontSize: '28px', fontWeight: 900, color: '#FF2D78', textShadow: '0 0 30px rgba(255,45,120,0.4)' }}>{prizePool.toFixed(5)} ETH</div>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', marginTop: '4px' }}>{Math.round(prizePool / 0.000125)} entries × 0.000125 ETH</div>
        </div>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '6px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr auto', padding: '10px 0 8px' }}>
        {['#', 'Player', 'Score'].map((h, i) => (
          <div key={i} style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', textAlign: i === 2 ? 'right' : 'left' }}>{h}</div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.2)', fontSize: '13px' }}>No players yet — be the first!</div>
